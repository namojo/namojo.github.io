...
옴니가 주장하는 바는 다릅니다. 텍스트와 이미지와 영상을 따로 처리하는 여러 모델을 이어 붙인 것이 아니라, **하나의 신경망이 포맷의 경계를 넘나들며 추론한다**는 것입니다. 업계에서는 이런 방향을 '세계 모델(world model)'이라고 부릅니다. 단어를 예측하는 것을 넘어, 물리적 세계가 어떻게 움직이는지 — 빛은 어떻게 떨어지고 사물은 어떻게 가려지는지 — 에 대한 내적 모형을 갖추려는 시도이지요.
