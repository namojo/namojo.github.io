...
| 2026-07-15 | 흰 화면 사고 수정 — Tailwind를 Play CDN에서 빌드타임 컴파일로 전환 | index.html, index.tsx, index.css (신규), tailwind.config.js (신규), postcss.config.js (신규) | 라이브 SPA가 흰 화면(+ `/index.css` 404) 신고. 원인: SPA 전체 스타일이 이제 사실상 폐기된 `cdn.tailwindcss.com` Play CDN 하나에 런타임 의존 + `<link href="/index.css">`가 존재하지 않는 파일을 가리켜 404. Play CDN이 브라우저에서 실패하면 스타일이 통째로 사라져 화면이 무너짐. 조치: 이미 설치돼 있던 tailwindcss@3.4.1로 빌드타임 컴파일로 전환 — 인라인 `tailwind.config`를 tailwind.config.js로 이관(ink/warm/coral·display 스케일·shadow·apple easing 그대로), postcss.config.js 추가, index.css(@tailwind 지시문) 생성 후 index.tsx에서 import, index.html에서 Play CDN 스크립트·인라인 config·죽은 index.css 링크 제거. vite가 해시된 CSS를 생성·주입. 새 npm 의존성 없음(prose/form-*/aspect-w-* 미사용 확인). 정적 포스트 페이지(build-static-posts.mjs)는 자체 인라인 CSS라 무관. 헤드리스 검증: body bg #FBFBFD, 히어로 72px(display-xl) 정상 적용, 콘솔 에러·404 0건. |
